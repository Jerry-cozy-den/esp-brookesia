...
#include "esp_lcd_st77916.h"     // ST77916 LCD驱动
...
/* LCD显示相关配置常量 */
#define LCD_CMD_BITS           32    /*!< LCD SPI命令位数：32位 */
...
esp_err_t bsp_camera_init(void)
{
    ...
    // 使用BSP已配置的I2C总线，不让摄像头驱动管理I2C引脚
    config.pin_sccb_sda = -1;               // 不管理I2C数据线引脚
    config.pin_sccb_scl = -1;               // 不管理I2C时钟线引脚
    config.sccb_i2c_port = BSP_I2C_NUM;      // 使用BSP配置的I2C端口号
