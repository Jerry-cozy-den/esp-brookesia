...
{
    *stack = (esp_gmf_job_stack_t *)esp_gmf_oal_calloc(1, sizeof(esp_gmf_job_stack_t));//没有进行类型转换。
...
static inline esp_gmf_err_t esp_gmf_job_stack_push(esp_gmf_job_stack_t *stack, uint32_t node_addr)
{
    ...
    esp_gmf_job_node_t *node = (esp_gmf_job_node_t *)esp_gmf_oal_calloc(1, sizeof(esp_gmf_job_node_t));//没有进行类型转换。
