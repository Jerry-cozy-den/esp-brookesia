...
#include "esp_brookesia_app_game_2048_phone.hpp"
...
Game2048 *Game2048::_instance = nullptr;

Game2048 *Game2048::requestInstance(bool use_status_bar, bool use_navigation_bar)
{
    if (_instance == nullptr) {
        _instance = new Game2048(use_status_bar, use_navigation_bar);
    }
    return _instance;
}

Game2048::Game2048(bool use_status_bar, bool use_navigation_bar):
    ESP_Brookesia_PhoneApp("2048", &img_app_2048, false, use_status_bar, use_navigation_bar)
{
    // 初始化默认值，实际尺寸在run()方法中获取
    _width = 480;  // 默认宽度
    _height = 320; // 默认高度
}
...
bool Game2048::run()
{
    ...
    // 获取屏幕尺寸
    _width = lv_obj_get_width(game_screen);
    _height = lv_obj_get_height(game_screen);

    // Initialize game state (moved from init function)
    ...
    lv_obj_set_style_bg_color(game_screen, GRID_BG_COLOR, 0);
    ...
    lv_obj_t *cur = lv_obj_create(game_screen);
    ...
    lv_obj_t *best = lv_obj_create(game_screen);
    ...
    lv_obj_t *grid = lv_obj_create(game_screen);
    ...
    _foreground_grid = lv_obj_create(game_screen);
    ...
    /* Add gesture detection for Phone system */
    lv_obj_add_event_cb(_foreground_grid, motion_event_cb, LV_EVENT_GESTURE, this);

    newGame();

    // 加载游戏屏幕到显示器
    lv_disp_load_scr(game_screen);
...
bool Game2048::close()
{
    ...
    lv_async_call([](void *user_data) {
        ...
        // Clean up gesture callback
        bool ret = lv_obj_remove_event_cb(app->_foreground_grid, motion_event_cb);
        if (!ret) {
